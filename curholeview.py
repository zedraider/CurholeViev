...
class MartingaleApp(tk.Tk):
    def __init__(self):
        ...
        self.langs = {
            "ru": {
                "title": f"CurholeView — симулятор мартингейла {self.cur_ver}",
                "deposit": "Депозит:",
            ...
            "en": {
                "title": f"CurholeView — Martingale Simulator ({self.cur_ver})",
                "deposit": "Deposit:",
                "max_bet": "Max bet (0=∞):",
    ...
    def _build_ui(self):
        ...
        style.configure(
            "Status.TLabel", font=("Segoe UI", 11, "bold"), foreground=colors["label"]
        )
        style.configure("Chart.TFrame", background=colors["chart_bg"])

        param_frame = ttk.LabelFrame(
            top, text=lang["control"], style="Param.TLabelframe"
        )
        param_frame.grid(
            row=0, column=0, sticky=tk.NSEW, padx=8, pady=8, ipadx=8, ipady=8
        )

        def add_label_entry(row, text, var, default, width=12):
            ttk.Label(param_frame, text=text, style="Param.TLabel").grid(
                row=row, column=0, sticky=tk.W, pady=3
            )
            var.set(default)
            e = ttk.Entry(
                param_frame, width=width, textvariable=var, style="Param.TEntry"
            )
            e.grid(row=row, column=1, sticky=tk.W, pady=3)
            return e

        self.deposit_var, self.max_bet_var = tk.StringVar(), tk.StringVar()
        self.deposit_entry = add_label_entry(
            0, lang["deposit"], self.deposit_var, "1000"
        )
        self.max_bet_entry = add_label_entry(
            1, lang["max_bet"], self.max_bet_var, "512"
        )

        (
            self.multiplier_var,
            self.delay_var,
            self.withdraw_per_win_var,
            self.withdraw_percent_var,
            self.win_prob_var,
        ) = [tk.DoubleVar() for _ in range(5)]
        ...
        ttk.Checkbutton(
            param_frame,
            text=lang["percent_on_win"],
            variable=self.apply_percent_on_win_var,
            style="Param.TCheckbutton",
        ).grid(row=7, column=0, columnspan=2, sticky=tk.W, pady=3)
        ...
        for idx, preset in enumerate(self.presets):
            ...
            btn = ttk.Button(
                presets,
                text=lang[preset["name"].lower()]
                if preset["name"].lower() in lang
                else preset["name"],
        ...
        batch_frame = ttk.LabelFrame(
            left_frame, text=lang["batch_sim"], style="Param.TLabelframe"
        )
        batch_frame.pack(fill=tk.X, **pad)
        ttk.Label(batch_frame, text=lang["batch_count"], style="Param.TLabel").pack(
            side=tk.LEFT, padx=5, pady=5
        )
        batch_spinbox = ttk.Spinbox(
            batch_frame, from_=1, to=1000, textvariable=self.batch_count_var, width=10
        )
        batch_spinbox.pack(side=tk.LEFT, padx=5, pady=5)
        ttk.Button(
            batch_frame,
            text=lang["start"],
            command=self.on_batch_start,
            style="Param.TButton",
        ).pack(side=tk.LEFT, padx=10, pady=5)

        btns = ttk.LabelFrame(
            left_frame, text=lang["control"], style="Param.TLabelframe"
        )
        btns.pack(fill=tk.X, **pad)
        for txt, cmd in [
            (lang["start"], self.on_start),
            (lang["stop"], self.on_stop),
            (lang["reset"], self.on_reset),
            (lang["save_csv"], self.save_csv),
            (lang["load_csv"], self.load_csv),
        ]:
            ttk.Button(btns, text=txt, command=cmd, style="Param.TButton").pack(
                side=tk.LEFT, padx=10, pady=10, ipadx=10, ipady=6
            )

        status = ttk.LabelFrame(
            left_frame, text=lang["status"], style="Param.TLabelframe"
        )
        ...
        charts = ttk.LabelFrame(
            right_frame, text=lang["charts"], style="Param.TLabelframe"
        )
        charts.pack(fill=tk.BOTH, expand=False, **pad)
        charts.configure(style="Chart.TFrame")

        # --- график депозит ---
        self.fig_bankroll = Figure(
            figsize=(6.5, 2.0), dpi=100, facecolor=colors["chart_bg"]
        )
        self.ax_bankroll = self.fig_bankroll.add_subplot(111)
        self.ax_bankroll.set_title(lang["deposit_title"], color=colors["chart_fg"])
        self.ax_bankroll.set_xlabel(
            lang["round"].split(":")[0], color=colors["chart_fg"]
        )
        ...
        self.canvas_bankroll.get_tk_widget().pack(
            fill=tk.NONE, expand=False, ipadx=0, ipady=0
        )
        self.bank_data = []
        ...
        self.canvas_bet.get_tk_widget().pack(
            fill=tk.NONE, expand=False, ipadx=0, ipady=0
        )
        self.bet_data = []
        ...
        # --- график накоплений ---
        self.fig_savings = Figure(
            figsize=(6.5, 2.0), dpi=100, facecolor=colors["chart_bg"]
        )
        self.ax_savings = self.fig_savings.add_subplot(111)
        self.ax_savings.set_title(lang["savings_title"], color=colors["chart_fg"])
        self.ax_savings.set_xlabel(
            lang["round"].split(":")[0], color=colors["chart_fg"]
        )
        self.ax_savings.set_ylabel(
            lang["savings"].split(":")[0], color=colors["chart_fg"]
        )
        ...
        self.canvas_savings.get_tk_widget().pack(
            fill=tk.NONE, expand=False, ipadx=0, ipady=0
        )
        self.savings_data = []
        self.x_data = []
    ...
    def _build_menu(self):
        ...
        settings_menu.add_command(
            label=lang["preset_settings"], command=self._edit_presets
        )
        ...
        help_menu.add_command(
            label=lang["about"],
            command=lambda: messagebox.showinfo(lang["about"], lang["about_text"]),
        )
    ...
    def _set_lang(self, lang_code):
        ...
        for i, btn in enumerate(self.preset_buttons):
            btn.config(
                text=lang[self.presets[i]["name"].lower()]
                if self.presets[i]["name"].lower() in lang
                else self.presets[i]["name"]
            )
    ...
    def _choose_theme(self):
        ...
        for theme_name, theme_data in self.theme_colors.items():
            ttk.Radiobutton(
                win, text=theme_name.capitalize(), variable=theme_var, value=theme_name
        ...
        ttk.Button(
            win, text=lang["apply"], command=lambda: [apply_theme(), win.destroy()]
        ).pack(pady=10)
    ...
    def _edit_presets(self):
        ...
        for i, preset in enumerate(self.presets):
            ...
            ttk.Label(frame, text=lang["preset_mult"]).grid(
            ...
            ttk.Label(frame, text=lang["preset_maxb"]).grid(
            ...
            ttk.Label(frame, text=lang["preset_delay"]).grid(
            ...
            ttk.Label(frame, text=lang["preset_withdraw_win"]).grid(
                row=3, column=0, sticky=tk.W, padx=5, pady=3
            )
            withdraw_win_var = tk.DoubleVar(value=preset.get("withdraw_win", 0.0))
            withdraw_win_entry = ttk.Entry(
                frame, textvariable=withdraw_win_var, width=10
            )
            withdraw_win_entry.grid(row=3, column=1, padx=5, pady=3)
            ttk.Label(frame, text=lang["preset_withdraw_percent"]).grid(
                row=4, column=0, sticky=tk.W, padx=5, pady=3
            )
            withdraw_percent_var = tk.DoubleVar(
                value=preset.get("withdraw_percent", 0.0)
            )
            withdraw_percent_entry = ttk.Entry(
                frame, textvariable=withdraw_percent_var, width=10
            )
            withdraw_percent_entry.grid(row=4, column=1, padx=5, pady=3)
            ttk.Label(frame, text=lang["preset_win_prob"]).grid(
            ...
            percent_on_win_var = tk.BooleanVar(
                value=preset.get("percent_on_win", False)
            )
            percent_on_win_cb = ttk.Checkbutton(
                frame, text=lang["preset_percent_on_win"], variable=percent_on_win_var
            )
            percent_on_win_cb.grid(
                row=6, column=0, columnspan=2, sticky=tk.W, padx=5, pady=3
            )
            entries.append(
                (
                    mult_var,
        ...
        def save():
            for i, (
                ...
                try:
                    ...
                    self.presets[i]["withdraw_percent"] = float(
                        withdraw_percent_var.get()
                    )
    ...
    def on_start(self):
        lang = self.langs[self.lang]
        try:
            self.bankroll = float(self.deposit_var.get())
            self.max_bet_limit = float(self.max_bet_var.get())
        except ValueError:
        ...
        self.status_msg.configure(text=lang["simulating"])
    ...
    def on_batch_start(self):
        ...
        self.status_msg.configure(
            text=f"{lang['simulating']} ({self.batch_count} {lang['batch_sim']})"
        )

    def on_stop(self):
        lang = self.langs[self.lang]
        self.stop_event.set()
        self.status_msg.configure(text=lang["stopping"])
    ...
    def _worker_loop(self):
        ...
        try:
            while not self.stop_event.is_set() and self.bankroll > 0:
                ...
                if self.max_bet_limit > 0:
                    bet = min(bet, self.max_bet_limit)
                ...
                if lose:
                    ...
                    if self.max_bet_limit > 0:
                        next_bet = min(next_bet, self.max_bet_limit)
                ...
                if p > 0.0:
                    if (not self.apply_percent_on_win_var.get()) or (
                        self.apply_percent_on_win_var.get() and not lose
                    ):
                ...
                self.results.append(
                    [self.round_index, self.bankroll, bet, self.savings_account, msg]
                )
            ...
            final_msg = (
                lang["stopped"] if self.stop_event.is_set() else lang["finished"]
            )
            self.ui_queue.put(
                {
                    "round": self.round_index,
    ...
    def _run_batch_simulations(self):
        ...
        for i in range(self.batch_count):
            ...
            # Запускаем симуляцию
            while (
                bankroll > 0 and round_index < 10000
            ):  # Ограничение на максимальное число раундов
                round_index += 1
                ...
                if lose:
                    ...
                    if withdraw_percent > 0:
                        if (not apply_percent_on_win) or (
                            apply_percent_on_win and not lose
                        ):
            ...
            if i % 10 == 0 or i == self.batch_count - 1:
                ...
                self.ui_queue.put(
                    {"progress": progress, "current": i + 1, "total": self.batch_count}
                )
        ...
        win_rate = (
            winning_sims / self.batch_count
        ) * 100  # Процент симуляций, закончившихся с прибылью

        # Выводим отладочную информацию
        print(
            f"Calculated stats: avg_profit={avg_profit}, avg_rounds={avg_rounds}, winning_sims={winning_sims}, loss_sims={loss_sims}, win_rate={win_rate}"
        )
    ...
    def _poll_queue(self):
        try:
            while True:
                self._apply_update(self.ui_queue.get_nowait())
        except queue.Empty:
            pass
    ...
    def _apply_update(self, u):
        ...
        if u.get("batch_complete"):
            ...
            try:
                ...
                print(
                    f"Parsed data: avg_profit={avg_profit}, avg_rounds={avg_rounds}, wins={wins}, losses={losses}, win_rate={win_rate}, batch_count={batch_count}, deposit={deposit}"
                )
            except Exception as e:
            ...
            stats_frame = ttk.LabelFrame(
                result_window, text=lang["batch_results"], padding=10
            )
            ...
            ttk.Label(
                left_col,
                text=lang["batch_avg_profit"] + f" {avg_profit:.2f}",
                font=("Segoe UI", 11),
            ).pack(anchor=tk.W, pady=5)
            ttk.Label(
                left_col,
                text=lang["batch_avg_rounds"] + f" {avg_rounds:.2f}",
                font=("Segoe UI", 11),
            ).pack(anchor=tk.W, pady=5)
            ttk.Label(
                left_col, text="Win rate: " + f"{win_rate:.2f}%", font=("Segoe UI", 11)
            ).pack(anchor=tk.W, pady=5)

            ttk.Label(
                right_col, text=lang["batch_wins"] + f" {wins}", font=("Segoe UI", 11)
            ).pack(anchor=tk.W, pady=5)
            ttk.Label(
                right_col,
                text=lang["batch_losses"] + f" {losses}",
                font=("Segoe UI", 11),
            ).pack(anchor=tk.W, pady=5)
            ttk.Label(
                right_col,
                text=lang["totalsim"] + f"{batch_count}",
                font=("Segoe UI", 11),
            ).pack(anchor=tk.W, pady=5)
            ttk.Label(
                right_col, text=lang["inidep"] + f"{deposit:.2f}", font=("Segoe UI", 11)
            ).pack(anchor=tk.W, pady=5)

            # Создаем текстовое поле для детальных результатов
            details_frame = ttk.LabelFrame(
                stats_frame, text="Detailed Results", padding=10
            )
            ...
            results_text = tk.Text(
                details_frame, height=10, width=50, yscrollcommand=scrollbar.set
            )
            ...
            results_text.insert(
                tk.END, "Simulation #\tFinal Balance\tSavings\tProfit\tRounds\n"
            )
            ...
            ttk.Button(
                button_frame, text=lang["apply"], command=result_window.destroy
            ).pack(pady=5)
        ...
        if u.get("progress"):
            ...
            total = u.get("total", 1)
            self.status_msg.configure(
                text=f"{lang['simulating']} ({current}/{total}) - {progress:.1f}%"
            )
            return
    ...
    def _rescale_axes(self):
        ...
        ymin, ymax = (
            (ymin - 1, ymax + 1) if ymin == ymax else (ymin * 0.95, ymax * 1.05)
        )
        ...
        ymin, ymax = (
            min(self.savings_data),
            max(self.savings_data) if self.savings_data else (0, 1),
        )
        ymin, ymax = (
            (ymin - 1, ymax + 1) if ymin == ymax else (ymin * 0.95, ymax * 1.05)
        )
    ...
    def load_csv(self):
        ...
        try:
            ...
            if last_entry:
                ...
                self.status_savings.configure(
                    text=lang["savings"].format(self.savings_account)
                )
